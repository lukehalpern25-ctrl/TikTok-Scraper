...
/**
 * Combines all emailFiltered CSV outputs into one master file
 */

const hashtags = ['lifeupdate', 'girlstalk', 'becomingher', 'justgirlythings'];
const outDir = path.resolve('../out');
const allRows = [];
let headers = [];

console.log('🔗 Combining all hashtag outputs into master CSV file...');

for (const hashtag of hashtags) {
  const filePath = path.join(outDir, `${hashtag}_emailFiltered.csv`);
  
  if (fs.existsSync(filePath)) {
    console.log(`📄 Reading ${hashtag}_emailFiltered.csv...`);
...
const combinedFileName = `combined_all_hashtags_${timestamp}.csv`;
