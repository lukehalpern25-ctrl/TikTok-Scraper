...
export function ProfileCard({ profile }: ProfileCardProps) {
  ...
  const getContactTypes = () => {
    ...
    if (linkIndicators.some(indicator => signature.toLowerCase().includes(indicator.toLowerCase()))) {
      contacts.push('👉 Link Indicator');
    }
    
    // Check for social links
    if (/\b(?:(?:https?:\/\/|www\.)[^\s<>()]+|(?:linktr\.ee|beacons\.ai|stan\.store|koji\.to|bio\.site|bio\.link|campsite\.bio|flow\.page|tap\.bio|msha\.ke)\/[^\s<>()]+)/i.test(signature)) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 h-full flex flex-col">
      ...
      <div className="flex items-start gap-3 mb-3">
        <img
          src={authorMeta.avatar || authorMeta.originalAvatarUrl || '/placeholder-avatar.png'}
        ...
        <div className="flex-1 min-w-0">
          ...
          {authorMeta.bioLink && (
            <a 
              href={authorMeta.bioLink.startsWith('http') ? authorMeta.bioLink : `${authorMeta.bioLink}`}
              target="_blank"
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="text-center">
          ...
          <div className="font-semibold text-sm text-gray-900">{formatNumber(authorMeta.fans || 0)}</div>
        </div>
        <div className="text-center">
          <div className="text-xs text-gray-500">Videos</div>
          <div className="font-semibold text-sm text-gray-900">{formatNumber(authorMeta.video || 0)}</div>
        </div>
        <div className="text-center">
          <div className="text-xs text-gray-500">Hearts</div>
          <div className="font-semibold text-sm text-gray-900">{formatNumber(authorMeta.heart || 0)}</div>
        </div>
      </div>
          ...
          <div className="w-full h-1 bg-gray-200 rounded-sm mt-1">
            <div 
              className={`h-full rounded-sm ${
                engagementRate > 3 ? "bg-green-500" : 
                engagementRate > 1 ? "bg-yellow-500" : "bg-red-500"
              }`}
      ...
      <div className="mt-auto pt-3 border-t border-gray-200">
        <a
          href={authorMeta.profileUrl || `https://www.tiktok.com/@${authorMeta.name}`}
          target="_blank"
          rel="noopener noreferrer"
